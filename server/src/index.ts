...
import { migrateWebSchema, RUNTIME_SCHEMA_VERSION, WEB_SCHEMA_VERSION } from './schema-migrations.js';
...
app.get('/internal/deep-health',async(req:any,reply)=>{if(!['127.0.0.1','::1','::ffff:127.0.0.1'].includes(String(req.ip||'')))return reply.code(403).send({error:'loopback_only'});const runtimeHealth=USE_AGENT_RUNTIME?await runtime.deepHealth():null;const migration=await db.get("SELECT COALESCE(MAX(version),0) version FROM schema_migrations WHERE owner='web'");const integrity=await db.get('PRAGMA integrity_check');const sqlite=integrity?.integrity_check==='ok',schemaMigrationVersion=Number(migration?.version||0),webSchemaCompatible=schemaMigrationVersion===WEB_SCHEMA_VERSION,runtimeSchemaCompatible=!!runtimeHealth&&Number(runtimeHealth.schemaMigrationVersion)===RUNTIME_SCHEMA_VERSION;const compatible=!!runtimeHealth&&Number(runtimeHealth.contractVersion)===API_DTO_CONTRACT_VERSION&&runtimeSchemaCompatible;return{ok:sqlite&&webSchemaCompatible&&compatible&&!!runtimeHealth?.ok,component:'web',releaseId:RELEASE_ID,contractVersion:API_DTO_CONTRACT_VERSION,schemaMigrationVersion,expectedSchemaMigrationVersion:WEB_SCHEMA_VERSION,webSchemaCompatible,runtimeSchemaCompatible,sqlite,runtimeConnected:!!runtimeHealth?.ok,runtimeReleaseId:runtimeHealth?.releaseId||null,runtimeMode:runtimeHealth?.mode||null,runtimeContractVersion:runtimeHealth?.contractVersion||null,runtimeSchemaMigrationVersion:runtimeHealth?.schemaMigrationVersion||null,compatible};});
