...
app.patch('/api/settings', { preHandler: ensureAuth }, async (req:any) => {
  ...
  if (Object.prototype.hasOwnProperty.call(req.body || {}, 'defaultModel')) await setSetting('defaultModel', cleanModel(req.body?.defaultModel) || '');
  return { settings: await appSettings() };
});
app.get('/api/models', { preHandler: ensureAuth }, async (req:any) => modelCatalog(req.query?.hidden === '1'));
...
app.post('/api/sessions', { preHandler: ensureAuth }, async (req:any, reply) => { let projectDir:string; try { projectDir = await validateProject(req.body?.projectDir, roots); } catch { return reply.code(400).send({error:'project path is outside allowed workspace roots'}); } const title = String(req.body?.title || path.basename(projectDir)); const settings = await appSettings(); const mode = normalizeMode(req.body?.mode) || settings.defaultMode; const model = cleanModel(req.body?.model) || settings.defaultModel; const opts = modeOptions(mode, model); const started = await codex.startThread(projectDir, opts); const thread = started.thread; await upsertThread(thread, { title, archived: 0, status:'idle', model, ...modeFields(mode) }); await codex.setName(thread.id, title).catch(()=>{}); return sessionDto(thread, { title, status:'idle', archived:0, model, ...modeFields(mode) }); });
...
async function appSettings() {
  ...
  return { defaultMode: normalizeMode(map.defaultMode) || 'yolo', defaultModel: cleanModel(map.defaultModel) || '' };
}
...
async function effectiveModel(row:any) { return cleanModel(row?.model) || (await appSettings()).defaultModel || undefined; }
