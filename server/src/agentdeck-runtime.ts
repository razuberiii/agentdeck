...
app.get('/internal/deep-health',async()=>{const migration=await db.get("SELECT COALESCE(MAX(version),0) version FROM schema_migrations WHERE owner='runtime'");const integrity=await db.get('PRAGMA quick_check(1)');const schemaMigrationVersion=Number(migration?.version||0),schemaShapeCompatible=await verifyRuntimeSchema(db).then(()=>true).catch(()=>false),schemaCompatible=schemaMigrationVersion===RUNTIME_SCHEMA_VERSION&&schemaShapeCompatible;return{ok:integrity?.quick_check==='ok'&&schemaCompatible,component:'runtime',releaseId:RELEASE_ID,contractVersion:API_DTO_CONTRACT_VERSION,schemaMigrationVersion,schemaCompatible,schemaShapeCompatible,sqlite:integrity?.quick_check==='ok',mode:RUNTIME_MODE,candidateRejectsTurns:RUNTIME_MODE==='candidate'};});
