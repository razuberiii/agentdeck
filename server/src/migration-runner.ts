...
export async function assertSchema(db:Db,owner:string,required:Record<string,string[]>){for(const[table,columns]of Object.entries(required)){const actual=new Set((await db.all(`PRAGMA table_info(${safeIdentifier(table)})`)).map(row=>String(row.name)));for(const column of columns)if(!actual.has(column))throw new Error(`${owner} schema incomplete: missing ${table}.${column}`);}}
