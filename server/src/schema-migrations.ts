...
const webRequired={tables:{users:['id','username','password_hash'],auth_sessions:['id','token_hash','expires_at'],sessions:['id','status','provider_id','model_revision'],runtime_ingestion_cursors:['session_id','committed_sequence','runtime_generation'],artifacts:['id','session_id','operation'],artifact_baselines:['session_id','turn_id','manifest_json'],codex_profiles:['id','status'],gemini_profiles:['id','status','default_model_mode'],antigravity_profiles:['id','status'],provider_login_attempts:['id','provider','status'],agent_messages:['id','session_id','client_message_id','status'],message_receipts:['session_id','client_message_id','status','retry_of'],interactive_requests:['request_id','session_id','status'],plan_tasks:['plan_id','session_id','status']},indexes:['auth_sessions_active','agent_messages_session_client_message','artifacts_turn_path_operation','message_receipts_retry_of','interactive_requests_session_status','plan_tasks_session_status']};
const runtimeRequired={tables:{sessions:['id','status','provider_id','upstream_thread_id','last_sequence'],events:['session_id','sequence','event_type','event_key'],accounts:['id','provider'],runtime_instances:['instance_id','heartbeat_at'],claude_profiles:['id','profile_dir','status'],plan_tasks:['plan_id','session_id','status']},indexes:['idx_events_session_sequence','idx_events_session_key','idx_events_session_type_sequence','idx_sessions_upstream_thread_id','plan_tasks_session_status']};
...
export async function migrateWebSchema(db:Db){await runMigrations(db,'web',[{version:1,name:'web_schema_baseline',statements:process.env.AGENTDECK_TEST_BAD_MIGRATION==='1'?['THIS IS INVALID SQL']:[]},webMigration,webMigration3,webMigration4]);await verifyWebSchema(db);}
export async function migrateRuntimeSchema(db:Db){await runMigrations(db,'runtime',[{version:1,name:'runtime_schema_baseline',statements:process.env.AGENTDECK_TEST_BAD_MIGRATION==='1'?['THIS IS INVALID SQL']:[]},runtimeMigration]);await verifyRuntimeSchema(db);}
export const WEB_SCHEMA_VERSION=4,RUNTIME_SCHEMA_VERSION=2;
