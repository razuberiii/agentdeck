...
const runtimeMigration4:Migration={version:4,name:'runtime_antigravity_cleanup_recovery',columns:{antigravity_terminal_intents:{cleanup_required:'INTEGER NOT NULL DEFAULT 0',process_group_id:'INTEGER',process_group_leader_pid:'INTEGER',process_group_start_ticks:'TEXT',cleanup_attempts:'INTEGER NOT NULL DEFAULT 0',cleanup_error:'TEXT'}},statements:[]};
const runtimeMigration5:Migration={version:5,name:'runtime_process_group_identity_and_turn_lineage',columns:{antigravity_terminal_intents:{process_group_members_json:'TEXT'}},statements:[
  'CREATE TABLE IF NOT EXISTS runtime_turn_lineage (session_id TEXT NOT NULL, turn_id TEXT NOT NULL, segment_id TEXT NOT NULL, client_message_id TEXT, message_id TEXT, retry_of TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, PRIMARY KEY(session_id,turn_id))',
  'CREATE INDEX IF NOT EXISTS runtime_turn_lineage_session ON runtime_turn_lineage(session_id,updated_at)'
]};
...
const webMigration7:Migration={version:7,name:'web_canonical_user_lineage',columns:{agent_messages:{retry_of:'TEXT',segment_id:'TEXT'}},statements:['CREATE INDEX IF NOT EXISTS agent_messages_session_turn ON agent_messages(session_id,turn_id,created_at)']};

const webRequired={tables:{users:['id','username','password_hash'],auth_sessions:['id','token_hash','expires_at'],sessions:['id','status','provider_id','model_revision'],runtime_ingestion_cursors:['session_id','committed_sequence','runtime_generation'],artifacts:['id','session_id','operation'],artifact_baselines:['session_id','turn_id','manifest_json'],turn_code_changes:['session_id','turn_id','changes_json'],codex_profiles:['id','status'],gemini_profiles:['id','status','default_model_mode'],antigravity_profiles:['id','status'],provider_login_attempts:['id','provider','status'],agent_messages:['id','session_id','client_message_id','turn_id','segment_id','status','retry_of'],message_receipts:['session_id','client_message_id','status','retry_of'],message_retry_claims:['session_id','retry_of','retry_client_message_id'],interactive_requests:['request_id','session_id','status'],plan_tasks:['plan_id','session_id','status']},indexes:['auth_sessions_active','agent_messages_session_client_message','agent_messages_session_turn','artifacts_turn_path_operation','turn_code_changes_session_created','message_receipts_retry_of','interactive_requests_session_status','plan_tasks_session_status']};
const runtimeRequired={tables:{sessions:['id','status','provider_id','upstream_thread_id','last_sequence'],events:['session_id','sequence','event_type','event_key'],accounts:['id','provider'],runtime_instances:['instance_id','heartbeat_at'],claude_profiles:['id','profile_dir','status'],plan_tasks:['plan_id','session_id','status'],antigravity_terminal_intents:['session_id','turn_id','terminal','terminal_payload_json','cleanup_required','process_group_id','process_group_start_ticks','process_group_members_json','cleanup_attempts','cleanup_error'],runtime_turn_lineage:['session_id','turn_id','segment_id','client_message_id','message_id','retry_of']},indexes:['idx_events_session_sequence','idx_events_session_key','idx_events_session_type_sequence','idx_sessions_upstream_thread_id','plan_tasks_session_status','idx_antigravity_terminal_intents_turn','runtime_turn_lineage_session']};
...
export async function migrateWebSchema(db:Db){await runMigrations(db,'web',[{version:1,name:'web_schema_baseline',statements:process.env.AGENTDECK_TEST_BAD_MIGRATION==='1'?['THIS IS INVALID SQL']:[]},webMigration,webMigration3,webMigration4,webMigration5,webMigration6,webMigration7]);await verifyWebSchema(db);}
export async function migrateRuntimeSchema(db:Db){await runMigrations(db,'runtime',[{version:1,name:'runtime_schema_baseline',statements:process.env.AGENTDECK_TEST_BAD_MIGRATION==='1'?['THIS IS INVALID SQL']:[]},runtimeMigration,runtimeMigration3,runtimeMigration4,runtimeMigration5]);await verifyRuntimeSchema(db);}
export const WEB_SCHEMA_VERSION=7,RUNTIME_SCHEMA_VERSION=5;
