...
export async function migrateRuntimeSchema(db:Db){await runMigrations(db,'runtime',[{version:1,name:'runtime_schema_baseline',statements:process.env.AGENTDECK_TEST_BAD_MIGRATION==='1'?['THIS IS INVALID SQL']:[]},runtimeMigration,runtimeMigration3,runtimeMigration4]);await verifyRuntimeSchema(db);}
export const WEB_SCHEMA_VERSION=7,RUNTIME_SCHEMA_VERSION=4;
