...
CREATE TABLE IF NOT EXISTS runtime_ingestion_cursors (session_id TEXT PRIMARY KEY, committed_sequence INTEGER NOT NULL DEFAULT 0, runtime_generation TEXT, updated_at INTEGER NOT NULL);
...
CREATE TABLE IF NOT EXISTS message_receipts (session_id TEXT NOT NULL, client_message_id TEXT NOT NULL, status TEXT NOT NULL, error TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, PRIMARY KEY(session_id,client_message_id));
...
CREATE TABLE IF NOT EXISTS login_attempts (ip TEXT PRIMARY KEY, count INTEGER NOT NULL, updated_at INTEGER NOT NULL);`);
  ...
  transactionRun(statements:Array<{ sql:string; params?:unknown[]; requireChanges?:number }>) {
    const db = this.open();
    return db.transaction(() => {
      ...
      for (const statement of statements) {
        ...
        if(statement.requireChanges!==undefined&&info.changes<statement.requireChanges)throw new Error(`transaction statement changed ${info.changes} rows; required ${statement.requireChanges}`);
