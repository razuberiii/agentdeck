...
function Home(){
  ...
  return <main className="appShell homeShell">
    ...
    <section className="homeHero">
      ...
      <div className={`launchAttachmentBar ${launchFiles.length?'hasFiles':''}`}><div className="launchAttachmentList">{launchFiles.map(item=><span className="attachmentChip" key={item.id}>{item.previewUrl?<img src={item.previewUrl} alt=""/>:<span className="attachmentFileIcon">{attachmentIconLabel({name:item.file.name,type:item.file.type})}</span>}<small><b>{item.file.name}</b><em>{formatSize(item.file.size)}</em></small><button type="button" aria-label={`移除 ${item.file.name}`} onClick={()=>removeLaunchFile(item.id)}>×</button></span>)}</div><div className="launchAttachmentActions" aria-label="添加附件"><button type="button" disabled={!canLaunchImages} onClick={()=>launchImageRef.current?.click()}><Icon name="attach" size={16}/><span><b>照片</b><small>图库与截图</small></span></button><button type="button" disabled={!canLaunchFiles} onClick={()=>launchFileRef.current?.click()}><Icon name="attach" size={16}/><span><b>文件</b><small>文档与源码</small></span></button></div><input ref={launchImageRef} hidden type="file" accept="image/*" multiple onChange={e=>{if(e.target.files)void addLaunchFiles(e.target.files);e.currentTarget.value='';}}/><input ref={launchFileRef} hidden type="file" multiple onChange={e=>{if(e.target.files)void addLaunchFiles(e.target.files);e.currentTarget.value='';}}/></div>
