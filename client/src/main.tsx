...
import { applyTimelineMessage, applyTimelineSnapshot, beginTimelineGeneration, emptyTimelineState, reconcileTimelineEvents, resolveTurnUiStatus, runtimeMessageSequence, type TimelineState, type TurnUiStatus } from './timeline-reducer';
...
import { BrowserOutbox, type OutboxRecord } from './browser-outbox';
import './styles.css';
...
function Home(){
  ...
  async function newSession(projectDir:string,title?:string,initialTask='',planMode:'direct'|'plan'=launchMode){
    ...
    setBusy(projectDir);
    try{ const s=await api('/api/sessions',{method:'POST',body:JSON.stringify({projectDir,title:title||projectName(projectDir),initialTask:initialTask.trim()||undefined,mode:status?.defaultMode,providerId:status?.activeProvider||'codex'})}); storageSet(sessionSendModeKey(s.id),planMode); if(initialTask.trim()){ storageSet(draftKey(s.id),initialTask.trim()); storageSet(pendingTaskKey(s.id),initialTask.trim()); storageSet(pendingTaskModeKey(s.id),planMode); } haptic(); location.hash='#/s/'+s.id; }
...
function SessionView({id}:{id:string}){
  ...
  function applyTimelineUpdate(fn:(state:TimelineState)=>TimelineState){ setTimeline(current=>{ const next=fn(current); snapshotCoveredSequenceRef.current=Math.max(snapshotCoveredSequenceRef.current,next.snapshotCoveredSequence); syncAppliedSequence(next.contiguousAppliedSequence); if(next.recovering) setRuntimeConnection('recovering'); return next; }); }
  async function load(resetLive=false,generation=sessionGenerationRef.current){ const startedAt=performance.now(); try{ const d=await api('/api/sessions/'+id); if(generation!==sessionGenerationRef.current){ console.info(`[perf] session-view-load ignored stale id=${id} ms=${Math.round(performance.now()-startedAt)}`); return clientAppliedSequenceRef.current; } const active=['running','planning','submitting','recovering','executing_approved_plan','waiting_approval','waiting_input','waiting_plan_approval'].includes(String(d.session?.activeTurn?.status || d.session?.status)); setSession(d.session); setPlans(Array.isArray(d.interactiveRequests)?d.interactiveRequests.filter((r:any)=>r?.kind==='plan_review'):[]); const covered=Number(d.snapshot?.throughSequence || d.snapshot?.coveredSequence || d.snapshot?.latestSequence || d.session?.last_sequence || 0); applyTimelineUpdate(state=>applyTimelineSnapshot(resetLive?{...state,liveMessages:[]}:state, threadEvents(d.thread), covered)); if(!hydratedRef.current||active){ hydratedRef.current=true; nearBottomRef.current=true; setShowBottom(false); scrollToBottomIfPinned(true); } setTurnStatus(resolveTurnUiStatus(d.session, [], false)); if(d.snapshot?.error) setRuntimeConnection(current=>current==='connected'?current:'recovering'); console.info(`[perf] session-view-load id=${id} gen=${generation} runtime=${runtimeConnection} ms=${Math.round(performance.now()-startedAt)}`); return Math.max(clientAppliedSequenceRef.current, covered); } catch(e:any){ if(generation===sessionGenerationRef.current) toast('error','读取会话失败：'+shortError(e)); return clientAppliedSequenceRef.current; } finally { if(generation===sessionGenerationRef.current) setLoading(false); } }
  function connect(generation=sessionGenerationRef.current){ if(!mountedRef.current) return; const proto=location.protocol==='https:'?'wss':'ws'; const ws=new WebSocket(`${proto}://${location.host}/ws`); wsRef.current=ws; const connectionGeneration=++connectionGenerationRef.current; setBrowserConnection(navigator.onLine?'reconnecting':'offline'); ws.onopen=()=>{ if(generation!==sessionGenerationRef.current || connectionGeneration!==connectionGenerationRef.current) return; const openedAt=performance.now(); setBrowserConnection('connected'); setRuntimeConnection('checking'); (async()=>{ const after=await load(true,generation); if(generation!==sessionGenerationRef.current || connectionGeneration!==connectionGenerationRef.current || ws.readyState!==WebSocket.OPEN) return; joinSentAtRef.current=performance.now(); ws.send(JSON.stringify({type:'join',sessionId:id,lastSequence:after,clientAppliedSequence:after,snapshotCoveredSequence:snapshotCoveredSequenceRef.current})); await recoverOutbox(ws); const pendingTask=storageGet(pendingTaskKey(id)); if(pendingTask?.trim()){ const pendingMode=storageGet(pendingTaskModeKey(id))==='plan'?'plan':'direct'; setSendMode(pendingMode); const clientMessageId=sendMessage(ws,id,{text:pendingTask.trim(),attachments:[],planMode:pendingMode}); pendingMessagesRef.current.set(clientMessageId,{text:pendingTask.trim(),attachments:[]}); storageRemove(pendingTaskKey(id)); storageRemove(pendingTaskModeKey(id)); storageRemove(draftKey(id)); setText(''); haptic(); } if(joinTimeoutRef.current) clearTimeout(joinTimeoutRef.current); joinTimeoutRef.current=window.setTimeout(()=>{ if(generation===sessionGenerationRef.current && connectionGeneration===connectionGenerationRef.current) setRuntimeConnection(current=>current==='checking'?'unavailable':current); },10000); console.info(`[perf] ws-open-to-join id=${id} gen=${generation} conn=${connectionGeneration} ms=${Math.round(performance.now()-openedAt)}`); refreshStatus(); })(); }; ws.onmessage=e=>applySocketMessage(JSON.parse(e.data),generation,connectionGeneration); ws.onclose=()=>{ if(wsRef.current===ws) wsRef.current=null; if(joinTimeoutRef.current) clearTimeout(joinTimeoutRef.current); if(generation===sessionGenerationRef.current && connectionGeneration!==connectionGenerationRef.current) return; if(joinTimeoutRef.current) clearTimeout(joinTimeoutRef.current); if(generation===sessionGenerationRef.current){ if(joinedOnceRef.current)runtimeRecoveryNeededRef.current=true; setRuntimeConnection('disconnected'); } setBrowserConnection(navigator.onLine?'reconnecting':'offline'); if(mountedRef.current && generation===sessionGenerationRef.current) reconnectRef.current=window.setTimeout(()=>connect(generation),1500); }; }
  function recoverRuntimeStream(nextGeneration:string,ws:WebSocket,generation:number,connectionGeneration:number){ if(runtimeRecoveryRef.current)return runtimeRecoveryRef.current; const targetGeneration=nextGeneration||runtimeGenerationRef.current; runtimeRecoveryNeededRef.current=false; setRuntimeConnection('recovering'); applyTimelineUpdate(state=>beginTimelineGeneration(state,targetGeneration||state.runtimeGeneration)); const task=(async()=>{ const through=await load(true,generation); if(generation!==sessionGenerationRef.current||connectionGeneration!==connectionGenerationRef.current||ws.readyState!==WebSocket.OPEN)return; if(targetGeneration)runtimeGenerationRef.current=targetGeneration; ws.send(JSON.stringify({type:'join',sessionId:id,lastSequence:through,clientAppliedSequence:through,snapshotCoveredSequence:through,runtimeGenerationRecovery:true})); setRuntimeConnection('checking'); })().finally(()=>{if(runtimeRecoveryRef.current===task)runtimeRecoveryRef.current=null;}); runtimeRecoveryRef.current=task; return task; }
  async function recoverOutbox(ws:WebSocket){ const records=await browserOutbox.list(id); for(const record of records){ if(["persisted","accepted","cancelled","failed"].includes(record.status)||(record.nextAttemptAt||0)>Date.now()) continue; try{ const status=await api(`/api/sessions/${encodeURIComponent(id)}/messages/${encodeURIComponent(record.clientMessageId)}/status`); await browserOutbox.update(record.clientMessageId,{status:status.status,lastError:status.error||undefined}); if(["persisted","accepted"].includes(status.status)){ await browserOutbox.removeBodyAfterPersisted(record.clientMessageId); continue; } }catch{} const attempts=record.attempts+1; sendMessage(ws,id,{text:record.text,attachments:record.attachments as Attachment[],planMode:record.planMode},record.clientMessageId); await browserOutbox.update(record.clientMessageId,{status:"sent",attempts,nextAttemptAt:Date.now()+browserOutbox.retryDelay(attempts)}); } }
