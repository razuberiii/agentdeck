...
function SessionView({id}:{id:string}){
  ...
  useEffect(()=>{let stopped=false,running=false;const reconcile=async()=>{if(running)return;running=true;try{const cutoff=Date.now()-10*60_000;const records=(await browserOutbox.list(id)).filter(record=>record.status==='accepted'&&Number(record.updatedAt||record.createdAt)>=cutoff).sort((a,b)=>Number(b.updatedAt||b.createdAt)-Number(a.updatedAt||a.createdAt)).slice(0,3);for(const record of records){if(stopped)return;try{const state=await api(`/api/sessions/${encodeURIComponent(id)}/messages/${encodeURIComponent(record.clientMessageId)}/status`);if(state.status==='failed')applySocketMessage({type:'messageStatus',clientMessageId:record.clientMessageId,status:'failed',error:state.error});}catch{}}}finally{running=false;}};const timer=window.setInterval(()=>void reconcile(),15_000);void reconcile();return()=>{stopped=true;window.clearInterval(timer);};},[id]);
