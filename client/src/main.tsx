...
function SessionView({id}:{id:string}){
  ...
  useEffect(()=>{ mountedRef.current=true; sessionGenerationRef.current++; const generation=sessionGenerationRef.current; const applied=Number(storageGet(sequenceKey(id)) || 0); clientAppliedSequenceRef.current=applied; snapshotCoveredSequenceRef.current=0; pendingMessagesRef.current=new Map(); setLoading(true); setTimeline(emptyTimelineState(applied)); setApprovals([]); setPlans([]); setSendMode(storageGet(sessionSendModeKey(id))==='plan'?'plan':'direct'); setRuntimeConnection('checking'); setTurnStatus('unknown'); setText(storageGet(draftKey(id)) || ''); setAttachments(loadDraftAttachments(id)); load(false,generation); refreshStatus(); connect(generation); const on=()=>{ const isOnline=navigator.onLine; setOnline(isOnline); setBrowserConnection(isOnline?(wsRef.current?.readyState===WebSocket.OPEN?'connected':'reconnecting'):'offline'); }; addEventListener('online',on); addEventListener('offline',on); return()=>{ mountedRef.current=false; removeEventListener('online',on); removeEventListener('offline',on); if(reconnectRef.current) clearTimeout(reconnectRef.current); if(joinTimeoutRef.current) clearTimeout(joinTimeoutRef.current); wsRef.current?.close(); }; },[id]);
