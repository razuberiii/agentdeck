...
function SessionView({id}:{id:string}){
  ...
  async function answerPlan(req:PlanRequest, optionId:'approve'|'revise'|'regenerate'|'cancel', text=''){ setBusy('plan:'+req.requestId+':'+optionId); try{ const answered=await api('/api/interactive-requests/'+encodeURIComponent(req.requestId)+'/answer',{method:'POST',body:JSON.stringify({optionId,text})}); setPlans(v=>v.filter(p=>p.requestId!==req.requestId)); if(optionId==='cancel') setSession(s=>s?{...s,status:'plan_cancelled',activeTurn:null}:s); haptic(); toast(optionId==='cancel'?'info':'success', optionId==='cancel'?'已取消计划':'已提交计划选择'); if(answered?.sentFollowup) setTurnStatus('running'); } catch(e:any){ toast('error','计划回复失败：'+shortError(e)); } finally{ setBusy(''); } }
  const rendered=visibleEvents(reconcileTimelineEvents([...(timeline.snapshotEvents as DisplayEvent[]),...liveEvents(timeline.liveMessages)])); const currentStatus=plans.length?'waiting_input':resolveTurnUiStatus(session, approvals, busy==='stop'||turnStatus==='cancelling', turnStatus, timeline.liveMessages); const activeModel=session?.model || (modelsProvider===(session ? sessionProvider(session) : '') ? catalogCurrent(models) : '') || status?.defaultModel; const runtimeUpdating=status?.runtimeState && status.runtimeState.acceptingNewTurns===false;
...
function liveEvents(items:any[]):DisplayEvent[]{
  ...
  for(const m of items){
    ...
    if(m.type==='user' && (String(m.text||'').trim() || m.attachments?.length)) {
      ...
      out.push({key:m.clientMessageId||m.messageId||'u'+out.length,clientMessageId:m.clientMessageId?String(m.clientMessageId):undefined,messageId:m.messageId?String(m.messageId):undefined,role:'user',text:stripInternalAttachmentPrompt(m.text||''),attachments:m.attachments||[],meta:messageStatusLabel(status)});
    }
...
function PlanCard({req,busy,edit,onEdit,onAnswer}:{req:PlanRequest;busy:boolean;edit:string;onEdit:(value:string)=>void;onAnswer:(req:PlanRequest,optionId:'approve'|'revise'|'regenerate'|'cancel',text?:string)=>void}){
  return <article className="approvalCard planCard" role="group" aria-label="计划确认">
    <div className="approvalHead"><b>{req.title || '执行计划'}</b><span>等待确认计划</span></div>
    <Markdown text={req.body || '计划内容为空'}/>
    <textarea className="planEdit" rows={3} value={edit} onChange={e=>onEdit(e.target.value)} placeholder="修改意见，可选"/>
    <div className="approvalActions planActions">
      <button disabled={busy} onClick={()=>onAnswer(req,'cancel')}>取消</button>
      <button disabled={busy} onClick={()=>onAnswer(req,'regenerate',edit)}>重新生成</button>
      <button disabled={busy || !edit.trim()} onClick={()=>onAnswer(req,'revise',edit)}>修改后执行</button>
      <button className="primary" disabled={busy} onClick={()=>onAnswer(req,'approve')}>{busy?'处理中':'批准并执行'}</button>
    </div>
  </article>;
}
