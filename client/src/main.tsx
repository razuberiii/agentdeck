...
function shortError(e:any){ return String(e?.userMessage || e?.message || e); }
...
function SessionView({id}:{id:string}){
  ...
  function applySocketMessage(msg:any,generation=sessionGenerationRef.current,connectionGeneration=connectionGenerationRef.current){ if(generation!==sessionGenerationRef.current || connectionGeneration!==connectionGenerationRef.current) return; if(!acceptRuntimeEvent(msg)) return; if(msg.type==='joined'){ if(joinTimeoutRef.current) clearTimeout(joinTimeoutRef.current); if(joinSentAtRef.current) console.info(`[perf] ws-join-to-connected id=${id} gen=${generation} conn=${connectionGeneration} ms=${Math.round(performance.now()-joinSentAtRef.current)}`); setRuntimeConnection(msg.runtimeConnection?normalizeRuntimeConnection(msg.runtimeConnection):'connected'); return; } if(msg.type==='runtimeConnection'){ if(joinTimeoutRef.current) clearTimeout(joinTimeoutRef.current); setRuntimeConnection(normalizeRuntimeConnection(msg.status)); markRuntimeApplied(msg); return; } if(msg.type==='messageStatus'){ const mid=String(msg.clientMessageId||''); if(msg.status==='failed'){ const pending=pendingMessagesRef.current.get(mid); if(pending){ setText(pending.text); setAttachments(pending.attachments); } toast('error','发送失败：'+shortError(msg.error||'runtime 未接受任务')); } if(msg.status==='accepted'||msg.status==='failed'||msg.status==='completed') pendingMessagesRef.current.delete(mid); setLive(v=>dedupeRuntimeMessages([...v,msg])); return; } if(msg.type==='thread_snapshot'){ if(msg.thread){ setEvents(threadEvents(msg.thread)); const covered=Number(msg.snapshot?.coveredSequence || msg.runtimeSequence || 0); snapshotCoveredSequenceRef.current=Math.max(snapshotCoveredSequenceRef.current, covered); setLive(v=>dedupeRuntimeMessages(v)); } if(msg.status) setTurnStatus(normalizeTurnStatus(msg.status)); markRuntimeApplied(msg); return; } if(msg.type==='approval'){ setApprovals(v=>v.some(a=>a.requestId===String(msg.requestId))?v:[...v,{requestId:String(msg.requestId),method:String(msg.method),params:msg.params}]); haptic(); toast('info','Codex 请求授权'); markRuntimeApplied(msg); return; } if(msg.type==='sessionTitle') setSession(s=>s?{...s,title:msg.title}:s); if(msg.type==='codex'&&msg.method==='turn/started'){ setSession(s=>s?{...s,status:'running'}:s); setTurnStatus('running'); } if(msg.type==='codex'&&msg.method==='turn/completed'){ const interrupted=turnFailed(msg.params?.turn); setSession(s=>s?{...s,status:interrupted?'interrupted':'idle'}:s); setTurnStatus(interrupted?'interrupted':'completed'); } if(msg.type==='codex'&&msg.method==='item/completed'&&isTerminalSignal(msg)){ setSession(s=>s?{...s,status:'idle'}:s); setTurnStatus('completed'); } if(msg.type==='codex'&&(msg.method==='turn/failed'||msg.method==='turn/interrupted')){ setSession(s=>s?{...s,status:'interrupted'}:s); setTurnStatus('interrupted'); } if(msg.type==='error'){ const pending=[...pendingMessagesRef.current.entries()].at(-1); if(msg.code==='runtime_draining'&&pending){ setText(pending[1].text); setAttachments(pending[1].attachments); pendingMessagesRef.current.delete(pending[0]); } toast('error', msg.code==='runtime_draining'?'系统正在更新，新任务暂时不可发送。':'请求失败：'+msg.error); } setLive(v=>dedupeRuntimeMessages([...v,msg])); markRuntimeApplied(msg); }
  function acceptRuntimeEvent(msg:any){ const seq=Number(msg.runtimeSequence||0); if(!seq) return true; const generation=String(msg.runtimeGeneration||'legacy'); const key=`${generation}:${seq}:${msg.type||''}:${msg.method||msg.status||''}`; if(seenRuntimeEventsRef.current.has(key)) return false; seenRuntimeEventsRef.current.add(key); return true; }
  function dedupeRuntimeMessages(items:any[]){ const seen=new Set<string>(); const out:any[]=[]; for(const item of items){ const seq=Number(item.runtimeSequence||0); const key=seq?`${item.runtimeGeneration||'legacy'}:${seq}:${item.type||''}:${item.method||item.status||''}`:`local:${out.length}`; if(seq&&seen.has(key)) continue; if(seq) seen.add(key); out.push(item); } return out; }
