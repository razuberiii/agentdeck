...
function Home(){
  ...
  async function newSession(projectDir:string,title?:string,initialTask=''){
  ...
  return <main className="appShell homeShell">
    ...
    <section className="homeHero">
      <div className="taskLaunch"><span className="heroKicker"><i/>NEW TASK</span><h1>今天想让 Agent<br/><em>完成什么？</em></h1><div className="taskPrompt"><textarea rows={4} value={taskText} onChange={event=>setTaskText(event.target.value)} onKeyDown={event=>{if((event.metaKey||event.ctrlKey)&&event.key==='Enter'&&taskText.trim())newSession(defaultWorkspace,'New task',taskText)}} placeholder="描述目标、上下文和期望结果…"/><footer><button className="workspacePill" aria-label="选择默认工作区" onClick={openProjectPicker}><Icon name="folder" size={15}/><span>{projectName(defaultWorkspace)}</span></button><span className="launchHint">⌘ Enter</span><button className="launchButton" disabled={!taskText.trim()||!!busy} onClick={()=>newSession(defaultWorkspace,'New task',taskText)}><span>{busy?'创建中':'开始任务'}</span><Icon name="arrow" size={17}/></button></footer></div><div className="heroMeta"><span>{online?'网络在线':'网络离线'}</span><span>{dashboard?.metrics.running?`${dashboard.metrics.running} 个任务运行中`:'当前没有运行任务'}</span><span>任务会在后台持续运行</span></div></div>
    ...
    <section className="quickStart">
      <button className="taskButton taskPrimary" disabled={!!busy} onClick={()=>newSession(defaultWorkspace,'Blank session')}><i><Icon name="spark" size={20}/></i><span>Blank session</span><b>{busy===defaultWorkspace?'正在创建…':'打开空白会话'}</b><small>先进入对话，再慢慢描述任务</small><Icon name="arrow"/></button>
...
function SessionView({id}:{id:string}){
  ...
  useEffect(()=>{ storageSet('agentdeck:sendMode', sendMode); },[sendMode]);
  ...
  return <main className={`chatShell ${drag?'dragging':''}`} onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);uploadFiles(e.dataTransfer.files)}}>
    ...
    <footer className="composer"><div className="composerSurface">
...
function PlanReviewCard({req,busy,onAnswer}:{req:PlanRequest;busy:boolean;onAnswer:(req:PlanRequest,optionId:string,note:string)=>void}){
  ...
  const secondary = req.options.filter(o=>o.id!==primary?.id);
  return <article className="approvalCard planReviewCard" role="group" aria-label="计划审核">
    <div className="approvalHead"><b>{req.title || '计划已生成'}</b><span>确认后继续实现，也可以补充要求让它重做计划。</span></div>
    <textarea className="planNote" rows={2} value={note} onChange={e=>setNote(e.target.value)} placeholder="可选：补充约束或修改意见"/>
    <div className="approvalActions planActions">
      {secondary.map(option=><button key={option.id} className={option.variant==='danger'?'dangerText':''} disabled={busy} onClick={()=>onAnswer(req,option.id,note)}>{option.label}</button>)}
      {primary&&<button className="primary" disabled={busy} onClick={()=>onAnswer(req,primary.id,note)}>{busy?'处理中':primary.label}</button>}
    </div>
  </article>;
}
