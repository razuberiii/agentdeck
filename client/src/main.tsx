...
function SessionView({id}:{id:string}){
  ...
  useEffect(()=>{const authoritative=authoritativeModelRef.current;if(session&&authoritative?.model&&session.model!==authoritative.model)setSession(current=>current?{...current,model:authoritative.model||undefined}:current);},[session?.model]);
  ...
  async function setSessionModel(model:string){const requestId=++modelMutationRef.current;setBusy('model');try{const applied=await api('/api/sessions/'+id,{method:'PATCH',body:JSON.stringify({model})});if(requestId!==modelMutationRef.current)return;const actual=String(applied.model||applied.modelId||model);authoritativeModelRef.current={model:actual,revision:Number(applied.modelRevision||requestId)};setSession(s=>s?{...s,model:actual}:s);setModelOpen(false);haptic();toast('success','已切换模型：'+modelLabel(actual));}catch(e:any){if(requestId===modelMutationRef.current)toast('error','模型切换失败：'+shortError(e));}finally{if(requestId===modelMutationRef.current)setBusy('');}}
