...
function currentAccountSummary(provider:ProviderId, profile:any, status?:ProviderStatus|null){
  ...
  if(state==='unresolved_identity' || status?.reasonCode==='codex_profile_identity_unresolved') return {primary:'账户信息读取中', secondary:''};
...
function SettingsSheet({data,loading,error,initialPage,onRetry,onChanged,onClose}:{data:any;loading:boolean;error:string;initialPage?:'main'|'agent';onRetry:()=>void;onChanged:()=>any|Promise<any>;onClose:()=>void}){
  ...
  useEffect(()=>{ if(!loginJob?.id || loginJob.status!=='running') return; const timer=window.setInterval(async()=>{ try{ const r=await api('/api/profile-login/'+loginJob.id); if(r.job.status!=='running'){ window.clearInterval(timer); if(r.job.status==='done') setLoginJob(null); else setLoginJob(r.job); await syncSettings(); toast(r.job.status==='done'?'success':'error', r.job.status==='done'?'登录完成':'登录未完成'); return; } setLoginJob(r.job); }catch{} },1500); return()=>window.clearInterval(timer); },[loginJob?.id,loginJob?.status]);
