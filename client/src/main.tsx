...
function SessionView({id}:{id:string}){
  ...
  useEffect(()=>{outboxSchedulerRef.current=new OutboxRetryScheduler(()=>browserOutbox.list(id),async()=>{const ws=wsRef.current;if(ws?.readyState===WebSocket.OPEN)await recoverOutbox(ws);});void outboxSchedulerRef.current.wake();return()=>{outboxSchedulerRef.current?.stop();outboxSchedulerRef.current=null;};},[id]);
  ...
  async function scheduleOutbox(){await outboxSchedulerRef.current?.wake();}
