...
function SessionView({id}:{id:string}){
  ...
  const [olderEvents,setOlderEvents]=useState<DisplayEvent[]>([]); const [historyCursor,setHistoryCursor]=useState(0); const [hasOlder,setHasOlder]=useState(false); const [loadingOlder,setLoadingOlder]=useState(false);
  ...
  const outboxRetryingRef=useRef(new Set<string>()); const loadingOlderRef=useRef(false); const olderEventsRef=useRef<DisplayEvent[]>([]);olderEventsRef.current=olderEvents;
  useEffect(()=>{ hydratedRef.current=false;nearBottomRef.current=true;authoritativeModelRef.current=null;modelMutationRef.current=0;resnapshotIdentitiesRef.current.clear();loadingOlderRef.current=false;setOlderEvents([]);setHistoryCursor(0);setHasOlder(false);setLoadingOlder(false);setShowBottom(false); },[id]);
  ...
  function onScroll(){ const el=feedRef.current; if(!el||!hydratedRef.current)return; nearBottomRef.current=el.scrollHeight-el.scrollTop-el.clientHeight<120; setShowBottom(!nearBottomRef.current); if(el.scrollTop<100&&hasOlder&&!loadingOlderRef.current)void loadOlder(); }
  ...
  async function stop(){
    if(busy==='stop')return;
    setBusy('stop');setTurnStatus('cancelling');
    try{
      await api('/api/sessions/'+encodeURIComponent(id)+'/stop',{method:'POST'});
      haptic();toast('info','已停止生成');
      applyTimelineUpdate(state=>applyTimelineMessage(state,{type:'system',text:'已停止生成'}));
      await load(false,sessionGenerationRef.current);
    }catch(e:any){
      toast('error','停止失败：'+shortError(e));
      await load(false,sessionGenerationRef.current);
    }finally{setBusy('');}
  }
  ...
  async function showQuota(){ setQuotaOpen(true); setQuota(null); try{ setQuota(await api('/api/quota?provider='+encodeURIComponent(session ? sessionProvider(session) : 'codex'),{signal:AbortSignal.timeout(12_000)})); } catch(e:any){ setQuota({errors:{rateLimits:e?.name==='TimeoutError'?'额度读取超时，请稍后重试':shortError(e)},checkedAt:Date.now()}); } }
  ...
  const conversationLive=liveEvents(timeline.liveMessages.filter(message=>message?.type!=='activity')); const activityLive=liveEvents(timeline.liveMessages.filter(message=>message?.type==='activity')); const rendered=visibleEvents(reconcileTimelineEvents([...olderEvents,...(timeline.snapshotEvents as DisplayEvent[]),...conversationLive,...outboxDisplayEvents(outboxRecords)])); const currentStatus=resolveTurnUiStatus(session, approvals, busy==='stop'||turnStatus==='cancelling', turnStatus, timeline.liveMessages); const sessionCapabilities=providerAttachmentCapabilities(status?.capabilities,session?sessionProvider(session):'codex'); const activeModel=session?.model || (modelsProvider===(session ? sessionProvider(session) : '') ? catalogCurrent(models) : '') || status?.defaultModel; const runtimeUpdating=status?.runtimeState && status.runtimeState.acceptingNewTurns===false; const planStage=plans.length?'计划等待确认':session?.status==='planning'?(rendered.some(event=>event.role==='assistant')?'正在整理执行路线':'正在检查仓库'):session?.status==='executing_approved_plan'?'正在按计划执行':''; const activityTurnId=String(session?.activeTurn?.turnId||'');const snapshotActivity=(timeline.snapshotEvents as DisplayEvent[]).filter(event=>(event.role==='command'||event.role==='file'||event.role==='reasoning')&&(!activityTurnId||String(event.turnId||'')===activityTurnId));const liveActivity=['running','planning','submitting','recovering','executing_approved_plan'].includes(String(session?.status||''))?mergeActivitySummary(snapshotActivity,activityLive).slice(-4):[];
  return <main ref={shellRef as any} className={`chatShell ${drag?'dragging':''}`} onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);uploadFiles(e.dataTransfer.files)}}>
    ...
    <footer ref={composerRef as any} className="composer"><div className={`composerSurface ${sendMode==='plan'?'planning':''}`}>
      ...
      <div className="composeRow"><div className="attachControl"><button className={`iconBtn attach ${attachMenu?'active':''}`} aria-label="添加附件" aria-expanded={attachMenu} onClick={()=>setAttachMenu(value=>!value)}><Icon name="attach"/></button>{attachMenu&&<div className="attachMenu"><button disabled={sessionCapabilities!==undefined&&!sessionCapabilities.imageInput} onClick={()=>{setAttachMenu(false);imageFileRef.current?.click()}}><b>照片</b><span>直接打开手机图库</span></button><button disabled={sessionCapabilities!==undefined&&!sessionCapabilities.fileInput} onClick={()=>{setAttachMenu(false);fileRef.current?.click()}}><b>文件</b><span>源码、PDF、压缩包等</span></button></div>}</div><textarea ref={textareaRef} rows={1} value={text} onPaste={e=>{const files=Array.from(e.clipboardData.files); if(files.length){e.preventDefault();uploadFiles(files)}}} onChange={e=>setText(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!isMobileInput()){e.preventDefault();send()}}} placeholder={runtimeUpdating?'系统正在更新，输入会保留':sendMode==='plan'?'计划模式：描述任务，只生成计划':'继续对话…'}/><button className="iconBtn stopButton" aria-label="停止生成" disabled={busy==='stop'} onClick={stop}><Icon name="stop"/></button><button className={`sendBtn ${runtimeUpdating?'updating':''}`} aria-label={runtimeUpdating?'系统更新中':'发送'} title={runtimeUpdating?'系统更新中':undefined} disabled={runtimeUpdating||busy==='send'||(!text.trim()&&!attachments.length)} onClick={send}>{runtimeUpdating?<Icon name="refresh" size={16}/>:busy==='send'?'发送中':<><span>发送</span><Icon name="send" size={16}/></>}</button></div>
...
function itemToEvent(item:any):DisplayEvent|null{
  if(item.type==='userMessage'){ const c=userContent(item.content); const text=stripInternalAttachmentPrompt(c.text); const attachments=dedupeAttachments([...(item.attachments||[]),...c.attachments]); return (text.trim() || attachments.length) ? {key:String(item.clientMessageId||item.id),messageId:String(item.id||''),clientMessageId:item.clientMessageId?String(item.clientMessageId):undefined,turnId:item.turnId?String(item.turnId):undefined,segmentId:item.segmentId?String(item.segmentId):undefined,retryOf:item.retryOf?String(item.retryOf):undefined,role:'user',text,attachments,meta:messageStatusLabel(item.status),deliveryStatus:item.status} : null; }
  ...
  if(item.type==='reasoning') return {key:item.id,turnId:item.turnId?String(item.turnId):undefined,segmentId:item.segmentId?String(item.segmentId):undefined,role:'reasoning',title:'思考摘要',text:[...(item.summary||[]),...(item.content||[])].join('\n')||'正在思考',open:false};
  ...
  if(item.type==='commandExecution') {
    ...
    return {key:item.id,turnId:item.turnId?String(item.turnId):undefined,segmentId:item.segmentId?String(item.segmentId):undefined,role:'command',title:'命令',meta:item.status,text:`$ ${item.command || ''}\n\n${output}`.trim(),open:failed};
  }
  if(item.type==='fileChange') {
    const failed = item.status && !['completed','success'].includes(item.status);
    const text = (item.changes||[]).map((c:any)=>c.path||c).filter(Boolean).join('\n');
    if(!text&&!failed)return null;
    return {key:item.id,turnId:item.turnId?String(item.turnId):undefined,segmentId:item.segmentId?String(item.segmentId):undefined,role:'file',title:failed?'文件修改失败':'文件已更新',meta:item.status,text:text||'文件修改失败',open:failed};
  }
...
function liveEvents(items:any[]):DisplayEvent[]{
  ...
  for(const m of items){
    ...
    if(m.type==='activity') out.push({key:`activity:${m.activityId}`,turnId:m.turnId,segmentId:m.segmentId,role:m.role||'reasoning',title:m.title||'处理中',text:String(m.detail||''),meta:m.phase||''} as DisplayEvent);
    ...
    if(m.type==='user' && (String(m.text||'').trim() || m.attachments?.length)) {
      ...
      out.push({key:m.clientMessageId||m.messageId||'u'+out.length,clientMessageId:m.clientMessageId?String(m.clientMessageId):undefined,messageId:m.messageId?String(m.messageId):undefined,turnId:m.turnId?String(m.turnId):undefined,segmentId:m.segmentId?String(m.segmentId):undefined,retryOf:m.retryOf?String(m.retryOf):undefined,role:'user',text:stripInternalAttachmentPrompt(m.text||''),attachments:m.attachments||[],meta:[messageStatusLabel(status),m.planMode==='plan'?'计划模式':''].filter(Boolean).join(' · '),deliveryStatus:status,deliveryError:m.clientMessageId?messageStatuses.get(String(m.clientMessageId))?.error:undefined});
    }
    ...
    if(m.type==='codex'){
      if(m.method==='item/agentMessage/delta'){
        ...
        if(delta){
          ...
          if(index==null){ streamingIndex.set(key,out.length); out.push({key,turnId:m.turnId||m.params?.turnId,segmentId:m.segmentId||m.params?.segmentId||m.turnId||m.params?.turnId,role:'assistant',text:delta,meta:'正在回复',images:extractMarkdownImages(delta),files:extractFileLinks(delta)}); }
...
function mergeActivitySummary(snapshot:DisplayEvent[],live:DisplayEvent[]){const byKey=new Map<string,DisplayEvent>();for(const event of[...snapshot,...live]){const identity=String(event.key||`${event.role}:${event.title}:${event.text}`);const key=identity.startsWith('activity:')?identity.slice('activity:'.length):identity;byKey.set(key,event)}return[...byKey.values()];}
...
function visibleEvents(items:DisplayEvent[]){
  ...
  return items.filter(e=>{
    ...
    if(e.role==='user'){
      ...
      if(text&&sameAttachments&&text===lastUserText&&!e.clientMessageId&&!e.messageId)return false;
      lastUserText = text;
...
function ChangeSetCard({title,changes}:{title:string;changes:Array<{status:string;path:string;toPath?:string}>}){
  ...
  const visible=expanded?changes:changes.slice(0,4),hidden=changes.length-visible.length;
  return <section className="changeSetCard"><header><span><i>∆</i><b>{title}</b></span><em>{changes.length} 个文件</em></header><div className="changeSummary">{counts.modified&&<span className="modified">修改 {counts.modified}</span>}{counts.added&&<span className="added">新增 {counts.added}</span>}{counts.deleted&&<span className="deleted">删除 {counts.deleted}</span>}{counts.renamed&&<span className="renamed">重命名 {counts.renamed}</span>}</div><div className="changeList">{visible.map((change,index)=><ChangeRow key={`${change.path}:${index}`} change={change}/>)}</div>{changes.length>4&&<button type="button" className={`changeToggle ${expanded?'expanded':''}`} onClick={event=>{setExpanded(value=>!value);event.currentTarget.blur();}}>{expanded?'收起文件列表':`展开其余 ${hidden} 个文件`}</button>}</section>;
}
