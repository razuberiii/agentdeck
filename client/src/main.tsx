...
function Home(){
  ...
  return <main className="appShell homeShell">
    ...
    {picker&&<ProjectPicker projects={projects} loading={projectsLoading} onRefresh={()=>loadProjects(true)} onClose={()=>setPicker(false)} onPick={(p)=>{setWorkspaceOverride(p.path);setPicker(false);haptic();toast('success',`任务目录已切换到 ${p.name}`)}}/>}
...
function SessionView({id}:{id:string}){
  ...
  const wsRef=useRef<WebSocket|null>(null); const reconnectRef=useRef<number|null>(null); const joinTimeoutRef=useRef<number|null>(null); const terminalSyncRef=useRef<number|null>(null); const mountedRef=useRef(false); const sessionGenerationRef=useRef(0); const connectionGenerationRef=useRef(0); const shellRef=useRef<HTMLElement|null>(null); const composerRef=useRef<HTMLElement|null>(null); const feedRef=useRef<HTMLElement|null>(null); const textareaRef=useRef<HTMLTextAreaElement|null>(null); const imageFileRef=useRef<HTMLInputElement|null>(null); const fileRef=useRef<HTMLInputElement|null>(null); const nearBottomRef=useRef(true); const clientAppliedSequenceRef=useRef(0); const snapshotCoveredSequenceRef=useRef(0); const joinSentAtRef=useRef(0); const pendingMessagesRef=useRef<Map<string,{text:string;attachments:Attachment[]}>>(new Map());
  ...
  const scrollToBottomIfPinned=()=>{ if(!nearBottomRef.current) return; const scroll=()=>{ const feed=feedRef.current; if(feed) feed.scrollTo({top:feed.scrollHeight}); }; requestAnimationFrame(()=>{ scroll(); requestAnimationFrame(scroll); }); window.setTimeout(scroll,120); };
  ...
  function syncTerminalSnapshot(generation=sessionGenerationRef.current){ if(terminalSyncRef.current) clearTimeout(terminalSyncRef.current); terminalSyncRef.current=window.setTimeout(()=>{ terminalSyncRef.current=null; if(mountedRef.current&&generation===sessionGenerationRef.current) void load(false,generation); },120); }
