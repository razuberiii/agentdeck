...
import {sameRecoveryRequest} from './runtime-recovery';
...
function Home(){
  ...
  return <main className="appShell homeShell">
    ...
    <section className="homeHero">
      ...
      <div className="launchAttachmentBar"><div>{launchFiles.map(item=><span className="attachmentChip" key={item.id}>{item.previewUrl?<img src={item.previewUrl}/>:<Icon name="attach" size={15}/>}<small>{item.file.name} · {formatSize(item.file.size)}</small><button aria-label={`移除 ${item.file.name}`} onClick={()=>removeLaunchFile(item.id)}>×</button></span>)}</div><button className="workspacePill" disabled={!status?.capabilities?.imageInput&&!status?.capabilities?.fileInput} onClick={()=>launchImageRef.current?.click()}><Icon name="attach" size={15}/>照片</button><button className="workspacePill" disabled={!status?.capabilities?.fileInput} onClick={()=>launchFileRef.current?.click()}><Icon name="attach" size={15}/>文件</button><input ref={launchImageRef} hidden type="file" accept="image/*" multiple onChange={e=>{if(e.target.files)void addLaunchFiles(e.target.files);e.currentTarget.value='';}}/><input ref={launchFileRef} hidden type="file" multiple onChange={e=>{if(e.target.files)void addLaunchFiles(e.target.files);e.currentTarget.value='';}}/></div>
...
function SessionView({id}:{id:string}){
  ...
  const frameGateRef=useRef(new DurableFrameGate()); const resnapshotEpochsRef=useRef(new Set<number>());
  ...
  function recoverRuntimeStream(nextGeneration:string,ws:WebSocket,generation:number,connectionGeneration:number){
    ...
    void (async()=>{const through=await load(true,generation);recovery.cursor=through;if(runtimeRecoveryRef.current!==recovery||generation!==sessionGenerationRef.current||connectionGeneration!==connectionGenerationRef.current||ws.readyState!==WebSocket.OPEN)return;if(targetGeneration)runtimeGenerationRef.current=targetGeneration;frameGateRef.current.reset(through,{connectionGeneration,joinRequestId,recoveryEpoch:epoch});ws.send(JSON.stringify({type:'join',sessionId:id,lastSequence:through,clientAppliedSequence:through,snapshotCoveredSequence:snapshotCoveredSequenceRef.current,runtimeGenerationRecovery:true,runtimeGeneration:targetGeneration,joinRequestId,recoveryEpoch:epoch}));})();
    return promise;
  }
  ...
  function applyDurableSocketMessage(msg:any,generation:number,connectionGeneration:number){if(msg?.type==='resnapshot_required'){const epoch=Number(msg.recoveryEpoch||0);if(resnapshotEpochsRef.current.has(epoch))return;resnapshotEpochsRef.current.add(epoch);const ws=wsRef.current;if(!ws)return;void(async()=>{const through=await load(true,generation),joinRequestId=crypto.randomUUID();frameGateRef.current.reset(through,{connectionGeneration,joinRequestId,recoveryEpoch:epoch});if(ws.readyState===WebSocket.OPEN)ws.send(JSON.stringify({type:'join',sessionId:id,lastSequence:through,clientAppliedSequence:through,snapshotCoveredSequence:through,joinRequestId,recoveryEpoch:epoch,runtimeGeneration:msg.runtimeGeneration||runtimeGenerationRef.current}));})();return;}const result=frameGateRef.current.push(msg);if(result.gap){const ws=wsRef.current;if(ws)void recoverRuntimeStream(String(msg.runtimeGeneration||runtimeGenerationRef.current),ws,generation,connectionGeneration);}for(const frame of result.ready){const recovery=runtimeRecoveryRef.current;if(runtimeMessageSequence(frame)&&recovery)runtimeRecoveryRef.current=null;applySocketMessage(frame,generation,connectionGeneration);if(recovery&&!runtimeRecoveryRef.current)runtimeRecoveryRef.current=recovery;}}
