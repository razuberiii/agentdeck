...
type ProviderInstaller = {provider:ProviderId;automatic:boolean;binary:string;packageName?:string|null;source:string;reason:string;manual:string;latestJob?:ProviderInstallJob|null};
type ProviderInstallJob = {id:string;provider:ProviderId;action:string;status:string;output:string[];error?:string|null;version?:string|null;startedAt:number;updatedAt:number};
...
type ActiveTurn = {turnId:string|null; status:string; startedAt?:number|null; waitingKind?:'approval'|'input'|null};
...
function statusLabel(s?:string){ return ({idle:'空闲',submitting:'提交中',running:'执行中',waiting_approval:'等待审批',waiting_input:'等待回答',cancelling:'正在停止',output_draining:'正在收尾',completed:'已完成',active:'空闲',failed:'失败',interrupted:'已中断',unknown:'未知',notLoaded:'可继续'} as any)[s||''] || s || '空闲'; }
