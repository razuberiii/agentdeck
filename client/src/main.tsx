...
function SessionView({id}:{id:string}){
  ...
  useEffect(()=>{void refreshOutboxView();outboxSchedulerRef.current=new OutboxRetryScheduler(()=>browserOutbox.list(id),async()=>{const ws=wsRef.current;if(ws?.readyState===WebSocket.OPEN)await recoverOutbox(ws);});void outboxSchedulerRef.current.wake();return()=>{outboxSchedulerRef.current?.stop();outboxSchedulerRef.current=null;};},[id]);
  ...
  async function retryOutbox(clientMessageId:string){const record=(await browserOutbox.list(id)).find(item=>item.clientMessageId===clientMessageId),ws=wsRef.current;if(!record||!ws||ws.readyState!==WebSocket.OPEN){toast('error','连接恢复后才能重试');return;}await browserOutbox.update(clientMessageId,{status:'ready',lastError:undefined,nextAttemptAt:0});await refreshOutboxView();await recoverOutbox(ws);}
