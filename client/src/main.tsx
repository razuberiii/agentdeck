...
function WorkPulse({dashboard,sessions}:{dashboard:Dashboard;sessions:Session[]}){
  ...
  const recent=sessions.filter(session=>!running.includes(session)).slice(0,3);
  return <section className="workPulse"><header><div><span>IN PROGRESS</span><b>{running.length?`${running.length} 件事正在处理`:'现在没有进行中的任务'}</b></div>{dashboard.metrics.waiting>0&&<i>{dashboard.metrics.waiting} 件需要处理</i>}</header>{running.length>0&&<div className="workPulseList">{running.map(session=><button key={session.id} onClick={()=>location.hash='#/s/'+session.id}><i className="liveDot"/><span><b>{displaySessionTitle(session)}</b><small>{projectName(session.project_dir)} · {statusLabel(session.status)}</small></span><Icon name="arrow" size={15}/></button>)}</div>}{recent.length>0&&<footer><span>刚刚完成或更新</span>{recent.map(session=><button key={session.id} onClick={()=>location.hash='#/s/'+session.id}>{displaySessionTitle(session)}<small>{formatTime(session.updated_at)}</small></button>)}</footer>}</section>;
}
