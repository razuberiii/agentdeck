...
export type Dashboard = { generatedAt:number; archived:boolean; metrics:{total:number;running:number;waiting:number;updatedToday:number;projects:number}; activity:{date:string;count:number}[]; projects:{path:string;name:string;sessions:number;lastActiveAt:number}[]; sessions:Session[] };
