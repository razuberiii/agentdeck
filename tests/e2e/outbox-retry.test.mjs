...
test('Antigravity Runtime failure is durably reflected in the Web receipt',async()=>{
  const port=await freePort(),runtimePort=await freePort(),root=await mkdtemp(path.join(tmpdir(),'agentdeck-retry-e2e-')),data=path.join(root,'data'),profileId='abcdef0123456789',home=path.join(data,'antigravity-profiles',profileId,'home'),counter=path.join(root,'turn-count'),binary=path.join(root,'agy');
  ...
  try{
    ...
    db.close();
    const runtimeDb=new Database(path.join(data,'agentdeck-runtime.sqlite3'));runtimeDb.prepare("INSERT INTO sessions(id,codex_thread_id,project_dir,title,status,permission_mode,approval_policy,sandbox_mode,model,archived,created_at,updated_at,provider_id,account_id,model_id,workspace_path,provider_session_id,provider,active_turn_id,last_sequence,interruption_reason,creator_profile_id,selected_profile_id,executing_profile_id,upstream_binding_profile_id,last_execution_account_id,current_upstream_account_id) VALUES(?,?,?,?,?,?,?,?,?,0,?,?,?,?,?,?,?,'antigravity',NULL,0,NULL,?,?,?,?,?,?)").run(sessionId,sessionId,process.cwd(),'Retry session','idle','workspace-write','on-request','workspace-write',null,now,now,'antigravity',profileId,null,process.cwd(),null,profileId,profileId,profileId,profileId,profileId,profileId);runtimeDb.close();
    ...
    assert.equal(await readFile(counter,'utf8'),'1','the Runtime must execute exactly one claimed turn');
    ...
    assert.equal(receipts.length,1);assert.equal(receipts[0].status,'failed');
