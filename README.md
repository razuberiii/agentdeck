...
- 支持项目 diff 查看和生成产物下载。
- Gemini CLI provider，通过 ACP 运行在持久 runtime 中。
...
AgentDeck runtime 使用 `gemini --acp` 启动长期 ACP 子进程。默认 profile 位于：

```text
/var/lib/agentdeck/gemini/profiles/default
```

Gemini 凭据与 Codex、Antigravity 分开保存，不会写入浏览器、SQLite 消息正文或公开仓库。首次使用前需要按 Gemini CLI 支持的方式完成 Google OAuth、API key、Vertex AI 或 gateway 配置。未登录时，设置页会显示 Gemini 已安装但 profile 需要认证。
