...
Gemini 凭据与 Codex、Antigravity 分开保存，不会写入浏览器、SQLite 消息正文或公开仓库。Web 登录当前支持 Gemini API Key：密钥只写入当前 profile 的 `agentdeck.env`，文件权限为 `0600`，profile 目录权限为 `0700`。Google OAuth 由 Gemini CLI 0.49.0 通过终端流输出授权 URL，ACP response 不提供稳定的远程网页桥接字段；远程服务器模式下请先通过 SSH 在对应 profile 的 `HOME` / `GEMINI_CONFIG_DIR` 中完成一次 `gemini` 登录，然后回到设置页重新检测。Vertex AI 和 gateway 只有在 Web 侧受控配置表单可用后才启用。
