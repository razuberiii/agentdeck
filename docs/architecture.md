...
SSE 订阅先注册为 replaying，再读取 durable watermark；注册窗口内提交的事件进入 subscriber buffer。完成 `(after, watermark]` 重放后只释放 watermark 之后的连续事件，出现缺口则断开并从 durable cursor 恢复。浏览器 Outbox 使用单一最近期限调度器，服务端终态不会因重连再次发送。
