...
`scripts/deploy.sh --check` performs validation without restarting services. Production deployment is a single operation with a lock, active-turn refusal, backup, health checks, and rollback. Codex app-servers are not restarted unless the deployment explicitly requires it and there is no active turn.
